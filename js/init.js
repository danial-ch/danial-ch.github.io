...
function arlo_tm_animate_text(){
	...
	animateSpan.typed({
		strings: ["Software Engineer", "Data Scientist", "Web Developer", "Machine Learning Engineer"],
		loop: true,
		startDelay: 1e3,
		backDelay: 2e3
	});
...
let currentSection = 'courses'; // Initially set to 'education'
let automaticSectionInterval;

function showSection(sectionName) {
	const cards = document.querySelectorAll('.card');
	const menuItems = document.querySelectorAll('.menuItem');
	cards.forEach(card => {
		if (card.id === sectionName) {
	...
	menuItems.forEach(item => {
		if (item.dataset.section === sectionName) {
			item.classList.add('selectedAcaItem'); // Add a class for the selected item
		} else {
			item.classList.remove('selectedAcaItem'); // Remove the class for non-selected items
		}
	});
}

function nextSection() {
	const sections = ['education', 'skills', 'extracurricular', 'tests', 'courses']; // Add other section names as needed
...
function startAutomaticSection() {
	automaticSectionInterval = setInterval(nextSection, 3000);
}

function stopAutomaticSection() {
	clearInterval(automaticSectionInterval);
}

// Initially show the 'education' section and set it as selected
showSection(currentSection);
const initialSelectedItem = document.querySelector(`[data-section="${currentSection}"]`);
if (initialSelectedItem) {
	initialSelectedItem.classList.add('selectedAcaItem');
}

function showSectionAndStop(section){
	showSection(section)
	stopAutomaticSection()
}

startAutomaticSection(); // Start automatic rotation
...
function showProject(n) {
  ...
  function startAutomaticSlide() {
	automaticSlideInterval = setInterval(nextSlide, 3000);
  }
...
// projectContainer.addEventListener('touchstart', touchStart, false);
// projectContainer.addEventListener('touchmove', touchMove, false);
